import { FacultyLocation, getRoomLocation } from "../../../lib/Room/RoomLocationApi";

// Mock location of faculty data
jest.mock('../../../assets/locationOfFaculty.json', () => ({
...
jest.mock('../../../lib/Room/RoomOSMLinkCreator', () => ({
...
jest.mock('../../../lib/Room/RoomAddressCreator', () => ({
  createRoomAddress: jest.fn((room: string, facultyLocation: FacultyLocation) => {
    if (room === 'WF-EX-2/252' && facultyLocation === 'WF') {
...
describe('RoomLocationApi', () => {
  describe('getRoomLocation', () => {
    it('should return correct location info for WF-EX-2/252 in WF faculty', () => {
      const locationInfo = getRoomLocation('WF-EX-2/252', 'Fakultät in Wolfenbüttel');
      expect(locationInfo).toEqual("WF-EX-2/252 (Am Exer 2, 38302 Wolfenbüttel) Link: https://osmapp.org/way/123456789");
    });

    it('should return correct location info for WF-EX-3/101 in WF faculty', () => {
      const locationInfo = getRoomLocation('WF-EX-3/101', 'Fakultät in Wolfenbüttel');
      expect(locationInfo).toEqual("WF-EX-3/101 (Am Exer 3, 38302 Wolfenbüttel) Link: https://osmapp.org/way/987654321");
    });
    ...
    it('should return correct location info for A068 in WF faculty', () => {
      ...
      expect(locationInfo).toEqual("A068 (Salzdahlumer Str. 46/48, 38302 Wolfenbüttel) Link: https://osmapp.org/way/123456789");
    });

    it('should return room name only for unknown room in WF faculty', () => {
      const locationInfo = getRoomLocation('UNKNOWN_ROOM', 'Fakultät in Wolfenbüttel');
